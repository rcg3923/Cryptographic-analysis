"""
=========================================
TC20 - Topy Cipher (encryption/decryption)
 - version 2: 
      동일한 라운드함수만 사용함
      키 스케줄 없음
      복호화 추가
=========================================
"""
import TC20_lib as TC20
import pickle  # 변수저장
import random  # 난수생성
import copy  # deep copy
import time  # 디버깅 할때 사용함

NUM_ROUND = 10

Sbox = [
    ,
]

ISbox = [
    ,
]


# --------
# AR: Add Roundkey
def AR(in_state, rkey):
    out_state = [0] * len(in_state)
    for i in range(len(in_state)):
        out_state[i] = in_state[i] ^ rkey[i]
    return out_state


# --------
# SB: Sbox layer
def SB(in_state):
    out_state = [0] * len(in_state)
    for i in range(len(in_state)):
        out_state[i] = Sbox[in_state[i]]
    return out_state


# --------
# LM: Linear Map
def LM(in_state):
    out_state = [0] * len(in_state)
    All_Xor = in_state[0] ^ in_state[1] ^ in_state[2] ^ in_state[3]
    for i in range(len(in_state)):
        out_state[i] = All_Xor ^ in_state[i]
    return out_state


# --------
# Enc_Round
def Enc_Round(in_state, rkey):
    out_state = [0] * len(in_state)
    out_state = AR(in_state, rkey)
    in_state = SB(out_state)
    out_state = LM(in_state)
    return out_state


# --------------------------------------
#  Decryption
# --------------------------------------


# -- SB: Sbox layer
def ISB(in_state):
    out_state = [0, 0, 0, 0]
    for i in range(len(in_state)):
        out_state[i] = ISbox[in_state[i]]
    return out_state


def int2list(n):
    out_list = []
    out_list.append((n >> 24) & 0xFF)
    out_list.append((n >> 16) & 0xFF)
    out_list.append((n >> 8) & 0xFF)
    out_list.append((n) & 0xFF)
    return out_list


# =================================== 위 내용은 실습때 사용했던 코드들 =============#


# ============= 여기부터 직접 제작 ======================================= #


# ====================================
def Extract_RK(P, C):
    out_state1 = [0, 0, 0, 0]
    out_state2 = [0, 0, 0, 0]

    out_state1 = LM(C)
    out_state2 = ISB(out_state1)

    RK = [0, 0, 0, 0]

    for i in range(len(out_state2)):
        RK[i] = out_state2[i] ^ P[i]
    return RK


# --------
def IsSlidPair(P1, C1, P2, C2):
    guess_key = Extract_RK(P1, P2)
    guess_C1 = Enc_Round(C1, guess_key)
    if C2 == guess_C1:
        # slid pair 가 맞으면 1
        return True
    else:
        # 아니면 0
        return False


# - 파일에서 변수를 가져오기 ( 이전에 쓰던 함수 긁어오기 )
def load_var_from_file(filename):
    f = open(filename, "rb")
    var = pickle.load(f)
    f.close()
    return var


def Slide_Attack():
    file_name = "known_ptct.var"
    pool = load_var_from_file(file_name)
    # exit_loop = False
    slid_pairs = []

    for i in range(len(pool)):
        print(".")
        for j in range(i + 1, len(pool)):
            P1 = pool[i][0]
            P2 = pool[j][0]
            C1 = pool[i][1]
            C2 = pool[j][1]

            if IsSlidPair(P1, C1, P2, C2) == 1:
                # exit_loop = True
                slid = Extract_RK(P1, P2)
                slid_pairs.append(slid)
                print("Success")

    print("추출한 키 값 : ", slid_pairs)


Slide_Attack()
