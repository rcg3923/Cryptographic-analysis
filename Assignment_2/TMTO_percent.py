...
def make_one_tmto_table(P0, m, t, ell):
    ...
    file_name = "tmto_extension_table/TMTO-" + str(ell) + ".dic"
...
for i in range(100):
    ...
    # P0 = 고정평문 [1,1,1,1]
    c_list.append(TC20.TC20_Enc(P0, key))
    c_list2.append(TC20.TC20_Enc(pt2, stack_key[i]))

    key_pool = []
    for ell in range(0, num_of_tables):
        # 후보키 리턴받는과정
        key_list = one_tmto_table_search(c_list[i], P0, m, t, ell)
        key_pool += key_list
        print(".", end="")

    final_key = []
    for key in key_pool:
        # 후보키로 만들어진 ct_result 와 다른 한쌍의 평문 같은 암호키로 만들어낸 다른 암호문 리스트의 0~99번 비교해서 같다면 final_key append를 해주는거임
        ct_result = TC20.TC20_Enc(pt2, key)
        if ct_result == c_list2[i]:
            final_key.append(key)
            succes_key_list.append(i)

percent = len(succes_key_list)
print(percent, "%")
